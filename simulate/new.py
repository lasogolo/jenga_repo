...
for k in range(2):
    ...
    for i in range(3):
        ...
        for j in range (3):
            ...
            new_pre_place_i = CartesianPose(0.3964+do+dx, 0.4487+dy, 0.23, radians(176.13), radians(2.01), radians(9.58))
            new_place_i = CartesianPose(0.3964+do+dx, 0.4487+dy, 0.21, radians(176.13), radians(2.01), radians(9.58))

            new_pre_pick_i = CartesianPose(-0.3093+dx_, 0.4304+dy_, 0.25+dz, radians(176.79), radians(0.50), radians(8.19+dr)) 
            new_pick_i = CartesianPose(-0.3093+dx_, 0.4304+dy_, 0.215+dz, radians(176.79), radians(0.50), radians(8.19+dr)) 
            ...
            new_pre_pick.append(new_pre_pick_i)
            new_pick.append(new_pick_i)

            new_pre_place.append(new_pre_place_i)
    ...
    with robot.connect("voraus-core", port=48401):
        ...
        for i in range (9):
            ...
            if i<9:
                ...
                robot.move_ptp(pre_pick_vec[i], blending=Percent(100) , speed=Percent(100)) # es posible que toce anadir joints
            ...
            robot.move_ptp(pre_pick_vec[i], blending=Percent(100), speed=Percent(100)) #y si fuera linear?
        ...
        robot.move_ptp(pre_pick, speed=Percent(100))
        for j in range(9):
            n = 9-1  
            #new pick
            robot.move_ptp(new_pre_place[n-j], blending=Percent(50) , speed=Percent(100))
            robot.move_linear_v(new_place[n-j], velocity_mps=2.0, extra_parameters=custom_parameters)
            tool_control_output.set()
            sleep(1) #pause,stop... docu

            robot.move_ptp(new_pre_place[j], blending= Percent(50), speed=Percent(100))
